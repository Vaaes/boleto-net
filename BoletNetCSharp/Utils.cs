...
namespace BoletNetCSharp
{
    ...
    {
        private static int _contador = 2;

        private static int _proximoNossoNumero = 1;

        internal static Cedente GerarCedente(string codigoCedente, string digitoCodigoCedente, string codigoTransmissao, ContaBancaria contaBancaria, Endereco endereco)
        {
            return new Cedente
            {
        ...
        internal static Sacado GerarSacado()
        {
            ...
            return new Sacado
            {
                ...
                Endereco = new Endereco
                {
                    LogradouroEndereco = "Avenida teste",
        ...
        internal static void TestarHomologacao(IBanco banco, TipoArquivo tipoArquivo, string nomeCarteira, int quantidadeBoletos, bool gerarBoletoPdfHtml, string aceite, int NossoNumeroInicial)
        {
            ...
            //Console.AreEqual(quantidadeBoletos, boletos.Count, "Quantidade de boletos diferente de " + quantidadeBoletos);
            ...
            if (File.Exists(nomeArquivoPDF))
            {
                ...
                if (File.Exists(nomeArquivoPDF))
                    Console.WriteLine("Arquivo Boletos (PDF) não foi excluído: " + nomeArquivoPDF);
            }
            if (File.Exists(nomeArquivoHTML))
            {
                File.Delete(nomeArquivoHTML);
                if (File.Exists(nomeArquivoHTML))
                    Console.WriteLine("Arquivo Boletos (HTML) não foi excluído: " + nomeArquivoHTML);
            }
            ...
            if (gerarBoletoPdfHtml)
            {
                ...
                try
                {
                    ...
                    foreach (var boletoTmp in boletos)
                    {
                        ...
                        if (!File.Exists(nomeArquivoPDF))
                            Console.WriteLine("Arquivo Boletos (PDF) não encontrado: " + nomeArquivoPDF);
